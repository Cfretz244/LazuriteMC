...
5. **Arming switch** — explicit arm/disarm override. **Keyboard step DONE (`5027cee`, pending fly-test)** — see §5. **Remaining:** map it to an **RC-controller button**, which needs button-binding support in the controller-setup screen (currently maps axes only). The keyboard step already built the arm/disarm machinery + motor-gating, so the controller path is just another input sending the `ARM_DISARM` packet.
